...
def stock_consumer_pipeline():
    ...
    dbt_run = make_dbt_operator("dbt_run", "run", "stocks")   # shared factory in shared/dbt_utils.py

    # dbt_test: checks not_null, unique, accepted_values, and singular tests on stocks models
    dbt_test = make_dbt_operator("dbt_test", "test", "stocks")  # same factory, test sub-command
    ...
    def detect_anomalies() -> dict:
        ...
        """
        import subprocess
        from shared.config import ANOMALY_CONTAMINATION, ANOMALY_N_ESTIMATORS  # deferred: tunable via env vars
        ...
        result = subprocess.run(
            [
                ...
                "--contamination", str(ANOMALY_CONTAMINATION),  # hyperparams from shared/config.py
                "--n-estimators", str(ANOMALY_N_ESTIMATORS),
            ],
            capture_output=True,
            text=True,
