...
The `anomaly_detector` DAG was already writing IsolationForest results to `PIPELINE_DB.ANALYTICS.FCT_ANOMALIES` in Snowflake, but there was no way to see those results without querying Snowflake directly. The dashboard is the natural place to show them — it connects what the ML pipeline produces with what a person actually needs to review.
