...
I saw it directly: after navigating to `/dashboard/`, all chart areas were blank for several seconds before populating. The delay was consistent and always tied to the first page load (subsequent loads were instant because the in-memory cache in `db.py` had already been populated).
...
`dcc.Loading` is Dash's built-in solution for exactly this problem. It tracks when any child component is waiting for a callback to complete and renders a spinner in its place. No extra state management, no polling, no custom JS — just wrapping the outputs.
