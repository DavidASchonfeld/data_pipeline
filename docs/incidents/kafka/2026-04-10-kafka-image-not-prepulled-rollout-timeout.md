...
`podReady=` being **empty** (not `True` or `False`) meant the pod had no `Ready` condition yet — the pod was in `ContainerCreating` or `Pending` state at the time of the check. It hadn't even started its container.

I looked at `scripts/deploy.sh` and saw that every other image that runs in K3s has an explicit pre-pull step:
...
`crictl pull` is idempotent — it does nothing if the image is already cached, and pulls only when missing. This separates image availability from the rollout timeout, so the 480-second window is used only for actual pod startup and the KRaft initialization startup probe.
...
- Matches the pattern of the existing airflow-dbt and MLflow pre-pull steps
