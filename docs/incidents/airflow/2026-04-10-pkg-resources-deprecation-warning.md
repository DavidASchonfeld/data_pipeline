...
The build output showed the warning on the `python -c "import pkg_resources"` line — the only place in the build that explicitly imports it. I checked the setuptools changelog and confirmed `pkg_resources` is deprecated API and the warning is intentional.
