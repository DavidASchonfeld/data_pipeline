...
The warning message named the exact path (`/tmp/.cache/pip`) and said it wasn't writable. I checked the deploy script and confirmed pip was invoked via `kubectl exec` — no `-H` flag, no explicit cache dir set — so pip tried to resolve a container-internal path it couldn't write to.
