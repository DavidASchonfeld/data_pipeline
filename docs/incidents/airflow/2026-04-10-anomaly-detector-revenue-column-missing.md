...
1. **urllib3 warning:** the warning text names the exact package and version. I traced the path (`/home/airflow/.local/lib/python3.12/site-packages/requests/__init__.py`) and confirmed it was Airflow's Python environment. The Dockerfile already had the fix, so this was just a deploy-lag issue.

2. **KeyError:** the traceback pointed to line 67 — the `groupby` column selector. I worked backward: the column would only be missing if the rename at line 58 did nothing, which would only happen if the pivot produced no `revenues` column, which would only happen if the SQL returned no revenue rows. I checked `edgar_client.py:44` and found the actual XBRL concept name (`RevenueFromContractWithCustomerExcludingAssessedTax`) used to populate Snowflake — it didn't match the filter string `'REVENUES'` at all.
