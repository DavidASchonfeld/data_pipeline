...
After deploying the Thread 1 ML pipeline, six issues were found while running through `docs/VERIFICATION.md` on EC2.
...
**Fix:** Documentation only — updated Step 5 in `VERIFICATION.md` to state that duplicates are expected and that the pass criterion is "appears at least once, no import errors."
...
**Fix:** Updated Step 8 in `VERIFICATION.md` to use the correct Airflow 3.x positional syntax:
```bash
...
**Fix:** Updated Step 9 in `VERIFICATION.md` with step-by-step Airflow UI navigation: DAG → run → `detect_anomalies` task → Logs tab → last stdout line (the JSON dict). The `mlflow_run_id` field in that JSON is what must match Snowflake.
...
**Files changed:** `docs/VERIFICATION.md`, `scripts/deploy/mlflow.sh`
