...
The `Init:` prefix in `Init:ErrImageNeverPull` told me an **init container** (specifically `wait-for-airflow-migrations`, which uses the same `airflow-dbt` image) couldn't find the image locally with `pullPolicy: Never`.

I confirmed the GC theory by looking at the api-server pod age progression in the pod listing:
...
The image was present immediately after import (`k3s ctr images list` verified it), but gone by the time Step 7 ran.
...
The check is quick — if the image is present, it's a single grep and the step completes in under a second. The re-import only runs when actually needed (rare, under disk pressure). This is the simplest fix that breaks the failure chain without disrupting the rest of the deploy sequence.
