...
The warning named the exact path (`/tmp/.cache/pip`). I checked the Dockerfile and confirmed that `USER airflow` is set before step `#10` while the prior venv RUN steps (`#7`, `#8`) run as root — so the permission conflict was unavoidable.
...
`--no-cache-dir` tells pip to skip all cache reads and writes. In a baked Docker image, the pip cache is never reused across builds (each `RUN` layer is isolated), so caching provides zero benefit here. Skipping it is the correct behavior, not a workaround. Alternatives — fixing `/tmp` permissions in the Dockerfile, setting `PIP_CACHE_DIR`, or reordering layers — would add complexity with no upside.
...
pip's own warning ("provide stricter constraints") pointed directly at the fix. I looked at the final installed versions in the build output (`dbt-adapters-1.9.0`, `dbt-common-1.12.0`) and confirmed that pinning those exact versions would let the resolver find the answer in a single lookup.
