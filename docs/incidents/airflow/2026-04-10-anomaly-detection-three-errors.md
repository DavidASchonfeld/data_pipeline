...
3. **`PermissionError`:** Full traceback showed the call chain: `mlflow.sklearn.log_model` → `log_artifacts` → `LocalArtifactRepo.log_artifacts` → `mkdir('/mlflow-data/...')`. The class `LocalArtifactRepo` told me the client resolved the artifact URI to a local filesystem path. That only happens when the tracking server returns a bare local path (`/mlflow-data/artifacts`) instead of an HTTP-proxied URI. Checking `deployment-mlflow.yaml` confirmed `--serve-artifacts` was absent from the server command.
