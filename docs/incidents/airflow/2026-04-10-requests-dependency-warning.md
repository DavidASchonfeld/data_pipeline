...
The warning path pointed directly at `requests/__init__.py:113` — the version compatibility check. I checked the `requests` changelog and confirmed the check was dropped in `2.32.0`, so the fix was to upgrade `requests` past that version.
