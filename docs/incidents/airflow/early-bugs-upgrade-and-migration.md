# Early Bugs: Upgrade and Migration

Historical record of bugs encountered during Airflow upgrades and infrastructure migration.
...
**Why it happened:** Both DAG files had a secret-validation block at module level that raised a `RuntimeError` during parsing when secrets weren't mounted yet. This crashed the DAG import, and the API server dropped the HTTP connection.
...
**The lesson:** Always use `--version` when running `helm upgrade`.
...
**Why it happened:** The chart referenced a secret called `airflow-webserver-secret-key` (a 2.x name). In 3.x, this was replaced with `airflow-api-secret-key`. The old secret didn't exist, so every pod failed to start — including the migration job, which meant the database never migrated, which meant init containers waited forever.
