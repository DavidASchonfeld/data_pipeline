...
During repeated deployments while integrating Kafka, MLflow, and the ML anomaly detector, I kept finding warnings during post-deploy debugging that had actually been present in earlier runs but got buried in the output. For example:
- A `WARNING: Helm did not update scheduler image` had been printed mid-output but I missed it, causing the next DAG run to use a stale image.
- pip `DEPRECATION:` lines from `ml-venv` installs turned into actual breakages later.

It became clear I needed a consolidated summary at the end of every deploy.
...
The `EXIT trap` pattern is the standard shell way to guarantee cleanup/reporting code runs. `tee` handles simultaneous live display + file capture. The two combined require no changes to individual steps while catching every warning source.
