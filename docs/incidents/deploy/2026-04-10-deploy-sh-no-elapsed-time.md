...
`deploy.sh` already printed a warning/error summary at the end of every run, but it never showed how long the deploy took. There was no way to tell at a glance whether a run finished in 2 minutes or 15 — I had to mentally track the start time or dig through the log timestamps.

## How It Was Encountered and Identified

I noticed during a routine deploy review: the summary block showed DEPLOY COMPLETE with warnings listed, but no timing information. Slow deploys (e.g., image builds, Helm upgrades) were impossible to distinguish from fast ones without external tooling.
...
`$SECONDS` is a bash built-in that auto-increments every second since the shell started — no `date` subprocess, no arithmetic on epoch strings, no portability concerns. It's the simplest way to measure how long a bash script takes.
