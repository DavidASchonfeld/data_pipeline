...
the failure look like it came out of nowhere with no useful debug info.
...
1. I saw "DEPLOY FAILED (exit code: 255)" with "(none)" warnings — those two things didn't add up.
2. I read `step_mlflow_portforward` in `mlflow.sh` and noticed `|| true` was scoped to the remote
   shell, not the local `ssh` invocation.
3. Confirmed SSH exit 255 = connection failure, not a kubectl error.
4. The port-forward step is purely a UI convenience (the SSH tunnel for MLflow); the
...
now tails the last 15 lines of the log instead of printing "(none)". This way there's always
something visible to debug, even for failures that don't produce recognizable keywords.
