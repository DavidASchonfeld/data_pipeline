...
The error message pointed directly to line 153 (the `ssh` call). Looking at that block, I spotted the unescaped `$1` inside a double-quoted string — a common shell quoting mistake.
...
The simplest correct fix for embedding `awk` field references inside a double-quoted SSH string is to escape the `$`. This preserves the double-quoted string (which is needed so other variables like `$BUILD_TAG` and `$EC2_HOME` *do* expand locally) while protecting the `awk`-specific `$1` from local expansion.
