...
During verification step 6 of the anomaly detection pipeline integration, immediately after running `./scripts/deploy.sh`. I entered the command in the terminal with a blank line between the backslash continuation and the `airflow tasks list` argument:
...
The two error messages gave it away:
...
Looking at the entered command alongside how POSIX continuation rules work made the blank line obvious as the cause.
