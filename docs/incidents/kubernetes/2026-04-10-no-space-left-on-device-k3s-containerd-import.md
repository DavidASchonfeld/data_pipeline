...
### 1. K3S containerd garbage collection never ran between deploys

`sudo k3s ctr images rm` removes only the image **manifest reference** from the K3S image index. The underlying content blobs (the actual layer data) stay on disk in `/var/lib/rancher/k3s/agent/containerd/io.containerd.content.v1.content/` until a garbage collection pass runs. The deploy script purged K3S image references but never triggered GC, so orphaned blobs piled up across every deploy until the content store ran out of space.

### 2. Old Docker airflow-dbt images not cleaned up

Each deploy creates a new timestamped Docker image (`airflow-dbt:3.1.8-dbt-YYYYMMDDHHMMSS`). The old images were never removed from Docker's image store, eating up additional disk space on the EC2 instance. The MLflow import step (2b5a) already ran `docker image prune -f` before its import — the airflow-dbt step was missing this.

---

## How It Was Identified

The error message pointed directly to the K3S containerd ingest path running out of space. I compared Step 2b2 (airflow-dbt) against Step 2b5a (MLflow) and saw that MLflow's step ran both `k3s ctr images rm` and `docker image prune -f` before importing, while airflow-dbt's step only ran `k3s ctr images rm` — no GC, no Docker prune.
