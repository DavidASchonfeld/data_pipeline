...
- No new timeout logic needed; piggybacking on the existing wait is simpler and more reliable
- Setting the same value every deploy is harmless (idempotent)

## How the Fix Solved the Problem

By moving the `airflow variables set` call to after `kubectl wait --for=condition=Ready` in Step 7, the exec only runs when the scheduler is confirmed healthy. The race condition between image import and pod recovery is gone.
