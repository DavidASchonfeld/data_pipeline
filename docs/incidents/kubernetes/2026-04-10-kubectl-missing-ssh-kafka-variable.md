...
The `kubectl exec` command that sets the Airflow variable was missing the `ssh "$EC2_HOST"` wrapper that every other `kubectl` call in the script uses. Without SSH, the command ran on my local Mac, which has no K3s API server. The Mac's kubectl tried to connect to `127.0.0.1:6443` (its default kubeconfig endpoint) and was refused.
...
The error message `dial tcp 127.0.0.1:6443: connect: connection refused` pointed to a local connection attempt. The Kafka section above it (which used `ssh "$EC2_HOST" "kubectl exec kafka-0 ..."`) succeeded, which confirmed the pattern: SSH-wrapped calls work, bare calls do not. I compared the failing block against surrounding blocks and found the missing `ssh`.
