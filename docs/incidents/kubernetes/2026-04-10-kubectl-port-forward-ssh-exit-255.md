...
Even though `kubectl port-forward` never reads from stdin, that open fd is enough to keep the SSH connection alive. The SSH client then waits indefinitely for EOF on the channel, eventually times out, and exits with code 255.

This is a common SSH + background process pitfall: stdout and stderr were correctly redirected to a file (`> /tmp/... 2>&1`), but **stdin was never closed**, leaving one channel still tied to the SSH session.
...
This is the standard pattern for launching background processes over SSH: make sure stdin, stdout, and stderr are all explicitly redirected before backgrounding.
