...
The `experiments/1` in the MLflow run URL told me the old experiment was still being used despite step 7c running. The `local_artifact_repo` class name in the traceback confirmed the artifact URI was still a local path, not a proxy URI. I looked at the restore block in `anomaly_detector.py` and realized that `get_experiment_by_name` searches all experiments (not just active ones) — that's how the stale experiment kept getting revived.
