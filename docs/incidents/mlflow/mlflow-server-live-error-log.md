...
## Why This Fix
An `initContainer` is the standard Kubernetes pattern for pre-flight setup on shared volumes. It runs to completion before the main container starts, is lightweight (busybox), and is scoped to this pod — no changes needed to the host, the image, or the PVC. The alternative (running the main container as root via `securityContext: runAsUser: 0`) works but is a broader security tradeoff; fixing permissions at the volume level is more targeted.
