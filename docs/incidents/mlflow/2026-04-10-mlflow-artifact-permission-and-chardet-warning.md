...
`mlflow-artifacts:/` is the standard MLflow way to configure artifact proxying. When `--serve-artifacts` is enabled and `--default-artifact-root` is `mlflow-artifacts:/`, the tracking server returns a proxied URI to clients, which then upload artifacts over HTTP to the server instead of writing directly to disk. The server stores the bytes at `--artifacts-destination`. This is the intended client/server separation — the scheduler pod never needs the PVC mounted.
