...
`kubectl rollout status` blocks until all replicas in the deployment pass their readiness probe (HTTP GET `/health` on port 5500). This is the standard Kubernetes way to wait for a deployment — it uses the same readiness signal that K8s uses for traffic routing, so by the time the command returns, port 5500 is guaranteed to be listening.
