...
Step 10 of `docs/VERIFICATION.md` (Access MLflow UI via SSH tunnel) failed with the above error. The error message ("dropped the connection" rather than "could not connect") indicated something was terminating the TCP connection immediately rather than refusing it outright — consistent with nothing listening on the target port.
