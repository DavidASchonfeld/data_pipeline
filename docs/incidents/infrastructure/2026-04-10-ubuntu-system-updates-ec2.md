...
`apt update && apt upgrade` is the standard way to apply pending Debian/Ubuntu package updates. It does not remove packages or change configuration files without prompting. It's safe to run and reversible via `apt-mark hold` if a specific package needs to be pinned.

## How the Fix Solved the Problem

Refreshing the package index (`apt update`) syncs the local cache with upstream mirrors so `apt` knows which newer versions are available. Running `apt upgrade` then downloads and installs those newer package versions. After the upgrade, the login banner no longer reports pending updates because all installed packages match their latest available versions.
