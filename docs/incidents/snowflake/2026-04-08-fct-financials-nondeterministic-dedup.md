...
When `filed_date` ties, the alphabetically earlier frame wins (e.g., `CY2024` before `CY2024Q4`). The choice is arbitrary but now consistent and deterministic across every run.

## Test added

Added `assert_fct_financials_no_duplicate_rows.sql` — a dbt test that fails if any `(ticker, metric, period_end)` combination has more than one row after dedup. This runs automatically with `dbt test --select tag:stocks` inside `dag_stocks.py`, so a future regression shows up as a DAG alert rather than a silent bad value.
