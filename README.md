...
The companion project is the counterpart: it queries the SEC's full company registry — around 10,000 entities — and processes a representative sample of 500 to demonstrate PySpark patterns at a scale where distributed processing is genuinely warranted (grouping, ranking, and window functions across hundreds of thousands of filing records). The 500-company sample is a deliberate cost/runtime bound chosen to match the architecture: the notebook uses `.cache()` to keep the working set resident across multiple aggregations, which is the right tool when the data fits in cluster memory and is reused — exactly the situation a 500-company sample creates.
